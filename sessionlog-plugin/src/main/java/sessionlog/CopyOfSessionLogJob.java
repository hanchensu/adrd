...
public class CopyOfSessionLogJob implements Tool {
	...
	private static final Log LOG = LogFactory.getLog(CopyOfSessionLogJob.class);
	
	public static void main(String[] args) throws Exception {
		
		int returnStatus = 0;
		CopyOfSessionLogJob job = new CopyOfSessionLogJob();
	...
	private void setupOptions() {
		...
		.addOption(configFile).addOption(libjars).addOption(numReduceTasks);
	}
...
//	    	System.out.println("config____________"+configFile_);
	    }
...
//			if (numReduce > 100) message = "reduce task number too large";
			System.out.println(numReduce+" "+message);
		}
	...
	private void setJobConf() throws IOException {
		config_.set("tmpfiles", configFile_);
